...
def make_kri_detail_export(drilldowns: list[dict[str, Any]], extra_sheets: dict[str, list[dict[str, Any]]] | None = None) -> bytes:
    output = BytesIO()
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for sheet_name, rows in (extra_sheets or {}).items():
            frame = pd.DataFrame(rows) if rows else pd.DataFrame([{"说明": "当前暂无数据"}])
            frame.to_excel(writer, sheet_name=safe_sheet_name(sheet_name, used_names), index=False)
