      ...
      <div v-if="step === 1" class="mapping-table">
        <div v-for="source in preview.sources" :key="source.source_id" class="mapping-row">
          ...
          <label>
            ...
            <AppSelect
              :model-value="selections[source.source_id]?.domain || ''"
              :options="[
                { value: '', label: t('mapping.skip') },
                ...preview.domains.map((domain) => ({ value: domain.key, label: domain.label })),
              ]"
              :aria-label="t('mapping.domain')"
              @update:model-value="onDomainChange(source.source_id, $event)"
            />
          </label>
        </div>
      </div>
          ...
          <section v-for="source in selectedSources" :key="source.source_id" class="field-map-card">
            ...
            <div class="field-map-grid">
              <label v-for="field in domainSpec(selections[source.source_id].domain)?.fields || []" :key="field" class="field-map-row">
                ...
                <AppSelect
                  v-model="selections[source.source_id].fields[field]"
                  :options="[
                    { value: '', label: '-' },
